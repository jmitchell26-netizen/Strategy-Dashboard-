  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h2 className="mt-1 text-2xl font-bold tracking-tight text-stone-100">Research Timeline</h2>
      ...
      {savedItems.length === 0 && (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-stone-800 py-24 text-center">
          ...
          <p className="mt-1 text-xs text-stone-600">Save articles in the Research tab to build your timeline.</p>
        </div>
      )}
        ...
        {grouped.map(([weekStart, items]) => (
          <div key={weekStart}>
            ...
            <div className="mb-4 flex items-center gap-3">
              ...
              <span className="shrink-0 rounded-full border border-stone-700 bg-stone-900 px-3 py-1 text-[11px] font-semibold text-stone-500">
            ...
            <div className="relative space-y-3 pl-6">
              {/* Vertical line */}
              <div className="absolute left-0 top-2 bottom-2 w-px bg-stone-800" />
              ...
              {items.map((item) => {
                ...
                return (
                  <div key={item.id} className="relative">
                    {/* Timeline node */}
                    <div className={`absolute -left-[19px] top-4 h-2.5 w-2.5 rounded-full border-2 border-stone-950 ${dot}`} />

                    <div className={`overflow-hidden rounded-xl border transition-colors ${
                      isOpen ? "border-stone-700 bg-stone-900" : "border-stone-800 bg-stone-900/60 hover:border-stone-700"
                    }`}>
                        ...
                        <div className="flex items-start gap-3">
                          <div className="flex-1 min-w-0">
                            {item.url ? (
                              <a href={item.url} target="_blank" rel="noopener noreferrer"
                                className="text-[13px] font-semibold leading-snug text-stone-200 hover:text-amber-400 transition-colors">
                                {item.title}
                              </a>
                            ) : (
                              <p className="text-[13px] font-semibold leading-snug text-stone-200">{item.title}</p>
                            )}
                          ...
                          <div className="flex shrink-0 items-center gap-1">
                            <button onClick={() => setExpandedId(isOpen ? null : item.id)}
                              className="rounded p-1 text-stone-500 hover:bg-stone-800 hover:text-stone-300 transition-colors">
                            ...
                            <button onClick={() => onRemove(item.id)}
                              className="rounded p-1 text-stone-600 hover:bg-red-500/10 hover:text-red-400 transition-colors">
                          ...
                          <div className="mt-4 space-y-3 border-t border-stone-800 pt-4">
                            <div>
                              ...
                              <Input value={item.companyName || ""} onChange={(e) => onUpdateCompany(item.id, e.target.value)}
                                placeholder="e.g. Apple" className="h-auto border-stone-700 bg-stone-800 px-3 py-1.5 text-[12px] text-stone-300 placeholder:text-stone-600 focus-visible:border-amber-600/50" />
                            </div>
                            <div>
                              ...
                              <Textarea value={item.notes || ""} onChange={(e) => onUpdateNotes(item.id, e.target.value)}
                                ...
                                className="resize-none border-stone-700 bg-stone-800 text-[12px] text-stone-300 placeholder:text-stone-600 focus-visible:border-amber-600/50" />
                            </div>
                          </div>
                        )}
                      </div>
