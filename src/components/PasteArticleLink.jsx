    ...
    <Card className="border-stone-200 bg-white">
      <CardContent className="p-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-stone-600">Add your own article</span>
          <span className="text-[11px] text-stone-400">— paste a link to bring it into the feed</span>
        </div>
        ...
        <form onSubmit={handleFetch} className="flex gap-2">
          <Input
            ...
            placeholder="https://…"
            autoComplete="off"
            className="flex-1 border-stone-200 bg-stone-50 text-sm text-stone-700 placeholder:text-stone-300 focus-visible:border-amber-400 focus-visible:ring-amber-200"
          />
          <Button type="submit" disabled={loading || !url.trim()} variant="outline" size="sm"
            className="shrink-0 border-amber-300 bg-amber-50 text-amber-700 hover:border-amber-400 hover:bg-amber-100">
        ...
        {showManual && url.trim() && (
          <form onSubmit={handleManualAdd} className="mt-3 space-y-2 rounded border border-stone-200 bg-stone-50 p-3">
            ...
            <Input type="text" value={manualTitle} onChange={(e) => setManualTitle(e.target.value)}
              placeholder="Title"
              className="border-stone-200 bg-white text-sm text-stone-700 placeholder:text-stone-300" />
            <Textarea value={manualDescription} onChange={(e) => setManualDescription(e.target.value)}
              ...
              className="resize-y border-stone-200 bg-white text-sm text-stone-700 placeholder:text-stone-300" />
            <Button type="submit" variant="outline" size="sm"
              className="border-stone-200 bg-white text-stone-600 hover:border-stone-300 hover:bg-stone-50">
              Add without preview
            </Button>
          </form>
        )}
