...
export default function StrategyMatrix({
  ...
  if (items.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded border border-dashed border-stone-200 bg-stone-50 py-16 text-center">
        <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded border border-stone-200 bg-white">
        ...
        <p className="mt-1 text-xs text-stone-400">Save articles from the news feed to get started</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      ...
      {/* ===== Compare toolbar ===== */}
      {items.length >= 2 && (
        <div className="flex items-center justify-between rounded border border-stone-200 bg-stone-50 px-4 py-2.5">
      ...
      {items.map((item, i) => {
        ...
        return (
          <div
            key={item.id}
            className={`animate-fade-in overflow-hidden rounded border transition-all duration-200 ${
              isSelected
          ...
          >
            ...
            <div className="p-4">
              ...
              <div className="mb-2.5 flex items-start gap-2.5">
                ...
                <h3 className="flex-1 text-[13px] font-semibold leading-snug text-stone-800">
                  {item.url ? (
                    <a href={item.url} target="_blank" rel="noopener noreferrer"
                      className="text-stone-800 decoration-stone-300 underline-offset-2 transition-colors hover:text-amber-700 hover:underline"
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      ...
                      className="shrink-0 rounded p-1 text-stone-300 transition-colors hover:bg-red-50 hover:text-red-500"
                    >
                      <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              ...
              <div className="mb-3 flex flex-wrap items-center gap-x-2 gap-y-1 pl-7 text-[11px]">
                <span className="inline-flex items-center gap-1.5 font-medium text-stone-500">
                  <span className={`h-1.5 w-1.5 rounded-full ${dot ?? "bg-stone-400"}`} />
                  {item.category}
                </span>
                <span className="text-stone-300">·</span>
                <span className="text-stone-400">{item.source}</span>
                <span className="text-stone-300">·</span>
                <span className="tabular-nums text-stone-400">{item.date}</span>
              </div>
              ...
              <div className="mb-3 pl-7">
                ...
                <Input
                  ...
                  className="h-auto rounded border-stone-200 bg-stone-50 px-3 py-1.5 text-[12px] text-stone-700 placeholder:text-stone-300 focus-visible:border-amber-400 focus-visible:ring-amber-200"
                />
              </div>
              ...
              <div className="pl-7">
                ...
                <Textarea
                  ...
                  rows={2}
                  className="resize-none rounded border-stone-200 bg-stone-50 px-3 py-2 text-[12px] leading-relaxed text-stone-700 placeholder:text-stone-300 focus-visible:border-amber-400 focus-visible:ring-amber-200"
                />
              </div>
            </div>
          </div>
