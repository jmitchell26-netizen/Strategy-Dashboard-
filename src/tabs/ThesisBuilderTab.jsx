  ...
  return (
    <div className="space-y-4 mt-4">
      {sections.map((s, i) => {
        ...
        return (
          <div key={i} className="border-t border-stone-800 pt-3">
              ...
              <div className="space-y-1.5">
                {bullets.map((b, j) => (
                  <div key={j} className="flex gap-2.5">
                    ...
                    <p className="text-[12px] leading-relaxed text-stone-400">{b}</p>
                  </div>
                ))}
              </div>
            ) : (
    ...
    <div className="grid gap-8 lg:grid-cols-[300px_1fr]">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-amber-600/80">Thesis Builder</p>
            <h2 className="mt-0.5 text-lg font-bold text-stone-100">Your Theses</h2>
          </div>
        ...
        {theses.length === 0 && (
          <div className="rounded-xl border border-dashed border-stone-800 py-12 text-center">
            <p className="text-sm text-stone-600">Create a thesis to start grouping articles.</p>
          </div>
        )}
              ...
              <div className="p-3">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button onClick={(e) => { e.stopPropagation(); deleteThesis(thesis.id); }}
                    className="rounded p-0.5 text-stone-600 hover:text-red-400 transition-colors">
      ...
      {!focused ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-stone-800 py-24 text-center">
          ...
          <p className="mt-1 text-xs text-stone-600">Click a thesis on the left to open the workspace.</p>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="rounded-xl border border-stone-800 bg-stone-900/60 p-5">
            <div className="mb-4 space-y-3">
              <div>
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-stone-600">Thesis Title</label>
                <input
                  value={focused.title}
                  onChange={(e) => updateThesis(focused.id, { title: e.target.value })}
                  className="w-full rounded-lg border border-stone-700 bg-stone-800 px-3 py-2 text-sm font-semibold text-stone-200 focus:border-amber-600/50 focus:outline-none"
                  placeholder="e.g. Apple is pivoting to services"
                />
              </div>
              <div>
                <label className="mb-1 block text-[10px] font-semibold uppercase tracking-wider text-stone-600">Hypothesis</label>
                <Textarea
                  ...
                  rows={3}
                  className="resize-none border-stone-700 bg-stone-800 text-[13px] text-stone-300 placeholder:text-stone-600 focus-visible:border-amber-600/50"
                />
              </div>
            </div>

            <div className="flex items-center gap-2 border-t border-stone-800 pt-4">
              <Button
                onClick={() => handleSynthesize(focused)}
                disabled={generatingId === focused.id || !isLlmConfigured() || focused.articleIds.length === 0}
                size="sm"
                className="bg-amber-500 text-stone-950 hover:bg-amber-400 disabled:opacity-40"
              >
          ...
          {focused.aiSummary && (
            <div className="rounded-xl border border-stone-800 bg-stone-900/60 p-5">
              <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-amber-600/80">AI Synthesis</p>
          ...
          <div className="rounded-xl border border-stone-800 bg-stone-900/60 p-5">
            ...
            {savedItems.length === 0 ? (
              <p className="text-[12px] text-stone-600">Save articles in the Research tab to add them here.</p>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      className={`flex cursor-pointer items-start gap-3 rounded-lg border p-3 transition-all ${
                        isAdded
                          ? "border-amber-700/50 bg-amber-500/8"
                    ...
                    >
                      <div className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border transition-colors ${
                        isAdded ? "border-amber-500 bg-amber-500" : "border-stone-600"
                      }`}>
                        {isAdded && <svg className="h-2.5 w-2.5 text-stone-950" fill="none" viewBox="0 0 24 24" strokeWidth={3} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg>}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] font-medium leading-snug text-stone-300 line-clamp-2">{item.title}</p>
                        <p className="mt-0.5 flex items-center gap-1.5 text-[10px] text-stone-600">
                          <span className={`h-1.5 w-1.5 rounded-full ${CAT_DOT[item.category] || "bg-stone-500"}`} />
